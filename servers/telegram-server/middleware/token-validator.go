package telegram_server_middleware
...
func TokenDecrypter(c *fiber.Ctx) error {
	...
	if _, ok := telegram_server_config.TelegramBotConfigMap.Tokens[tokenString[0]]; !ok {
		c.Locals(interfaces.REQ_LOCAL_ERROR_KEY, &interfaces.RequestError{
			StatusCode: 401,
		...
		c.Locals(telegram_server_config.REQ_LOCAL_NUMBER_KEY, tokenString[0])
	}
	return c.Next()
}

func AllowOnlyValidTokenMiddleWare(c *fiber.Ctx) error {
	jwtRawFromLocal := c.Locals(telegram_server_config.REQ_LOCAL_NUMBER_KEY)
...
func AllowOnlyValidLoggedInWhatsapp(c *fiber.Ctx) error {
	...
	if value, ok := telegram_server_config.TelegramBotConfigMap.Tokens[token]; !ok {
		return &interfaces.RequestError{
			StatusCode: 401,
			Code:       interfaces.ERROR_INVALID_TOKEN,
			Message:    "Invalid Token",
