...
type (
	apiSendMessage struct {
		To  []int64 `json:"to" validate:"required,dive,min=1"`
		Msg string  `json:"msg"`
	}
...
func SendMessage(c *fiber.Ctx) error {
	...
	if len(body.To) == 0 || len(strconv.FormatInt(body.To[0], 10)) < 7 {
	...
	connection := telegram_config.ConnectionMap.GetBotConnection(token)
	if connection == nil {
		return &interfaces.RequestError{
	...
	if runHeadLess {
		go connection.SendMessage(body.To, body.Msg)
		return c.JSON(fiber.Map{
			"success": true,
		})
	} else {
		return c.JSON(connection.SendMessage(body.To, body.Msg))
	}
}
