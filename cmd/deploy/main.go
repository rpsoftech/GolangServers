...
func main() {
	...
	for _, target := range targets {
		for compName, compPath := range components {
			version := getNextVersion(deployEnv, compName, target.OS, target.Arch, kvToken)
...
func getNextVersion(envName, comp, osName, arch, token string) int {
...
func uploadFile(path, filename, uploadPathFolder, token string) error {
	...
	writer.Close() // Must close BEFORE getting the final byte slice!

	// 2. Extract final bytes and calculate EXACT size
	payloadBytes := payload.Bytes()
	exactSize := int64(len(payloadBytes))

	// 3. Create request using bytes.NewReader to prevent chunked streaming
	req, err := http.NewRequest("POST", FileServerURL+filename, bytes.NewReader(payloadBytes))
	if err != nil {
		return err
	}

	// 4. Force the Content-Length header so Node.js accepts it immediately
	req.ContentLength = exactSize
	req.Header.Set("Content-Length", fmt.Sprintf("%d", exactSize))
